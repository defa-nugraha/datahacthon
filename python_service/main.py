...
def build_vertex_client():
    if genai is None:
        raise RuntimeError("google-genai belum terpasang.")
...
def generate_vertex_care_advice(payload: CareAdviceRequest) -> dict[str, Any]:
    if os.getenv("VERTEX_AI_ENABLED", "true").lower() != "true":
        raise RuntimeError("Vertex AI dinonaktifkan lewat konfigurasi.")

    if not os.getenv("GOOGLE_CLOUD_PROJECT") and not os.getenv("VERTEX_API_KEY"):
        raise RuntimeError("Konfigurasi Vertex AI belum lengkap.")
...
""".strip()
    ...
    response_schema = {
        ...
        "properties": {
            "summary": {
                "type": "STRING",
                "description": "Ringkasan singkat kondisi zona dan tindakan utama dalam bahasa Indonesia."
            },
            "urgency": {
                "type": "STRING",
                "enum": ["low", "medium", "high"],
                "description": "Tingkat urgensi tindakan berdasarkan histori 1 jam terakhir."
            },
            "recommendations": {
                "type": "ARRAY",
                "description": "Daftar aksi praktis yang bisa dijalankan di lapangan.",
                "items": {
                    "type": "OBJECT",
                    "properties": {
                        "title": {"type": "STRING", "description": "Judul aksi singkat."},
                        "detail": {"type": "STRING", "description": "Penjelasan aksi dan alasan agronomisnya."},
                        "priority": {"type": "STRING", "enum": ["low", "medium", "high"], "description": "Prioritas aksi."},
                        "timing_hours": {"type": "NUMBER", "description": "Perkiraan kapan aksi sebaiknya dilakukan, dalam jam."}
                    },
                    "required": ["title", "detail", "priority", "timing_hours"]
                }
            },
            "observation_focus": {
                "type": "ARRAY",
                "items": {"type": "STRING"},
                "description": "Hal yang harus dipantau operator setelah tindakan dijalankan."
            },
            "risk_flags": {
                "type": "ARRAY",
                "items": {"type": "STRING"},
                "description": "Risiko utama yang terdeteksi dari histori nutrisi dan kelembapan."
            }
        },
        "required": ["summary", "urgency", "recommendations", "observation_focus", "risk_flags"],
    }
...
def care_advice(payload: CareAdviceRequest) -> dict[str, Any]:
    try:
        return generate_vertex_care_advice(payload)
    except Exception as exc:
        logger.warning("Vertex AI unavailable, using fallback care advice: %s", exc)
