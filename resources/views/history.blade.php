...
@section('title', 'Analysis History - Vera AI')
    ...
    <div class="mx-auto max-w-7xl space-y-6">
        <div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                <a href="{{ route('zones.index') }}" class="rounded-2xl bg-primary px-5 py-3 text-sm font-semibold text-white transition hover:bg-success">Run New Analysis</a>
            </div>
        </div>
                ...
                <section class="rounded-3xl border border-outline bg-surface p-6 shadow-panel">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div class="mt-1 text-5xl font-extrabold tracking-tight text-primary">{{ number_format($historicalData->getCollection()->avg('soil_moisture') ?? 0, 0) }}%</div>
                        </div>
                    </div>
                    ...
                    <tbody class="divide-y divide-outline">
                        @forelse ($historicalData as $data)
                            ...
                            <tr class="hover:bg-surface-soft/70">
                                <td class="px-6 py-5 text-slate-700">{{ optional($data->sampled_at ?? $data->created_at)->format('Y-m-d H:i') }}</td>
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-3">
                                        <span class="inline-block h-2.5 w-2.5 rounded-full bg-primary"></span>
                                        <span class="font-semibold text-slate-900">{{ $data->zone?->name ?? 'Zona tak dikenal' }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-5 text-slate-600">
                                    <div>pH: <span class="font-semibold text-slate-900">{{ number_format($data->ph, 1) }}</span></div>
                                    <div>NPK: {{ number_format($data->nitrogen, 0) }}-{{ number_format($data->phosphorus, 0) }}-{{ number_format($data->potassium, 0) }}</div>
                                </td>
                                <td class="px-6 py-5 font-semibold text-primary">{{ $recommendedPlant }}</td>
                                <td class="px-6 py-5">
                                    <span class="rounded-full px-3 py-1 text-xs font-semibold {{ $statusClass }}">{{ $actionLabel }}</span>
                                </td>
                                <td class="px-6 py-5 text-right">
                                    @if ($data->zone)
                                        <a href="{{ route('zones.show', $data->zone) }}" class="font-semibold text-primary hover:text-success">Lihat</a>
                                    @else
...
@push('scripts')
    ...
    @endphp
    <script>
        const historyRows = @json($historyChartRows);
